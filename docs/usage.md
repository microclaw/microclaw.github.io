...
`*` `bot_username` can be overridden per channel with optional `channels.<name>.bot_username` and, for Telegram multi-account mode, `channels.telegram.accounts.<id>.bot_username`.
